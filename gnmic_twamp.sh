...
latest=$(gnmic -a 172.20.20.107 -u grpc -p telemetry --insecure get --format flat --path "/state/oam-pm/session[session-name=toCPE]/ip/twamp-light/statistics/delay/measurement-interval[duration=5-mins]/newest-index" | awk -F': ' '{print $2}')
completed=$((latest-1))
